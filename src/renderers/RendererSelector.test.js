...
test('maps block_quote nodes to BlockQuoteRenderer', () => {
  const blockQuoteNode = getTestingNode('> this is a block quote');
  const renderer = RendererSelector(blockQuoteNode);
  expect(renderer instanceof BlockQuoteRenderer).toBe(true);
});
...
test('maps link nodes to LinkRenderer', () => {
  const linkNode = getTestingNode('Here is a [link](https://fsf.org)')
    .lastChild;
